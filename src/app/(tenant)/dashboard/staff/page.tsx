...
export default function StaffPage() {
  ...
  const [staffList, setStaffList] = useState<StaffItem[]>(STAFF_DATA);
  ...
  const [weeklySchedule, setWeeklySchedule] = useState([
    ...
    { dayOfWeek: 0, dayName: 'Sunday', startTime: '10:00', endTime: '16:00', isWorking: false },
  ]);

  const handleToggleScheduleWorking = (idx: number) => {
    setWeeklySchedule((prev) => {
      const updated = [...prev];
      updated[idx] = { ...updated[idx], isWorking: !updated[idx].isWorking };
      return updated;
    });
  };

  const handleScheduleTimeChange = (idx: number, field: 'startTime' | 'endTime', val: string) => {
    setWeeklySchedule((prev) => {
      const updated = [...prev];
  ...
  // Store business hours boundaries for constraint validation
  const STORE_BUSINESS_HOURS: Record<number, { openTime: string; closeTime: string; isOpen: boolean }> = {
    1: { openTime: '09:00', closeTime: '21:00', isOpen: true }, // Monday
    2: { openTime: '09:00', closeTime: '21:00', isOpen: true }, // Tuesday
    3: { openTime: '09:00', closeTime: '21:00', isOpen: true }, // Wednesday
    4: { openTime: '09:00', closeTime: '21:00', isOpen: true }, // Thursday
    5: { openTime: '09:00', closeTime: '21:00', isOpen: true }, // Friday
    6: { openTime: '09:00', closeTime: '21:00', isOpen: true }, // Saturday
    0: { openTime: '10:00', closeTime: '18:00', isOpen: true }, // Sunday
  };

  const handleSaveScheduleTimetable = async () => {
    ...
    for (const item of weeklySchedule) {
      ...
      if (item.startTime >= item.endTime) {
        showToast.error(
          `Invalid Shift Time on ${item.dayName}`,
          `Start time (${item.startTime}) must be strictly before end time (${item.endTime}).`
        );
        return;
      }

      // 2. Validate against store operating hours
      const storeHp = STORE_BUSINESS_HOURS[item.dayOfWeek];
      if (storeHp) {
        if (!storeHp.isOpen) {
          showToast.error(
            `Store Closed on ${item.dayName}`,
            `The store is closed on ${item.dayName}. Staff cannot be scheduled.`
          );
          return;
        }

        if (item.startTime < storeHp.openTime || item.endTime > storeHp.closeTime) {
          showToast.error(
            `Store Hours Constraint Exceeded`,
            `Working hours on ${item.dayName} (${item.startTime} - ${item.endTime}) must stay within store hours (${storeHp.openTime} - ${storeHp.closeTime}).`
          );
          return;
        }
      }
    ...
    try {
      ...
      if (selectedStaff?.id) {
        ...
        await staffApiService.updateStaffSchedule(selectedStaff.id, payload);
      }

      // Update working hours text for selected staff
      const workingDays = weeklySchedule.filter((d) => d.isWorking);
      if (workingDays.length > 0) {
        const sample = workingDays[0];
        setSelectedStaff((prev) => ({
          ...prev,
          workingHours: `${sample.startTime} - ${sample.endTime}`,
        }));
      }

      setIsAssignScheduleOpen(false);
      showToast.success(
        'Schedule Saved Successfully!',
        `Weekly timetable for ${selectedStaff.name} has been updated in the database.`
      );
    } catch (error: any) {
      const msg = error?.response?.data?.message || error?.message || 'Failed to update staff schedule';
      showToast.error(
  ...
  const handleOpenProfile = async (staff: StaffItem) => {
    setSelectedStaff(staff);
    setIsProfileDrawerOpen(true);

    try {
      if (staff?.id) {
        const schedules = await staffApiService.getStaffSchedule(staff.id);
        if (schedules && schedules.length > 0) {
          ...
          const loaded = [1, 2, 3, 4, 5, 6, 0].map((d) => {
            const found = schedules.find((s) => s.dayOfWeek === d);
            return {
              dayOfWeek: d,
              dayName: dayNamesMap[d],
              startTime: found ? found.startTime : '09:00',
              endTime: found ? found.endTime : '18:00',
              isWorking: found ? found.isWorking : d !== 0,
            };
          });
      ...
      // Fallback to default schedule state if network error
    }
  };
        ...
        <Box sx={{ width: '100%', maxWidth: '100%', pb: 4 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flexWrap: 'wrap' }}>
              ...
              <Button
                ...
                sx={{
                  ...
                  color: (theme) => (theme.palette.mode === 'dark' ? '#F8FAFC' : '#374151'),
                  fontWeight: 600,
                  fontSize: '0.84rem',
                  py: 0.9,
                  px: 2,
          ...
          >
            ...
            <TableContainer>
              <Table size="small">
                <TableHead>
                  <TableRow sx={{ '& th': { borderBottom: (theme) => (theme.palette.mode === 'dark' ? '1px solid rgba(255, 255, 255, 0.08)' : '1px solid #F3F4F6'), py: 1.2 } }}>
                    <TableCell padding="checkbox">
                      <Checkbox size="small" />
                    </TableCell>
                    <TableCell sx={{ fontWeight: 700, color: 'text.secondary', fontSize: '0.75rem' }}>Staff</TableCell>
                    <TableCell sx={{ fontWeight: 700, color: 'text.secondary', fontSize: '0.75rem' }}>Role</TableCell>
                    <TableCell sx={{ fontWeight: 700, color: 'text.secondary', fontSize: '0.75rem' }}>Phone</TableCell>
                    <TableCell sx={{ fontWeight: 700, color: 'text.secondary', fontSize: '0.75rem' }}>Email</TableCell>
                    <TableCell sx={{ fontWeight: 700, color: 'text.secondary', fontSize: '0.75rem' }}>Next Shift</TableCell>
                    ...
                    return (
                      <TableRow
                        ...
                        sx={{
                          ...
                          '& td, & th': { borderBottom: (theme) => (theme.palette.mode === 'dark' ? '1px solid rgba(255, 255, 255, 0.05)' : '1px solid #F3F4F6') },
          ...
          >
            ...
            {activeDrawerTab === 0 && (
              <>
                    ...
                    <Card sx={{ borderRadius: '16px', border: '1px solid #F3F4F6', p: 2, backgroundColor: '#FFFFFF', height: '100%' }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1.5 }}>
                        ...
                        <Button
                          ...
                          onClick={() => setIsAssignScheduleOpen(true)}
                          sx={{ textTransform: 'none', fontWeight: 700, fontSize: '0.72rem', color: '#7C3AED' }}
                        >
                          Edit Schedule
                        </Button>
                      ...
                      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                        ...
                        <Box><Typography variant="caption" color="text.secondary" sx={{ fontSize: '0.72rem' }}>Working Hours</Typography><Typography variant="body2" sx={{ fontWeight: 600, fontSize: '0.8125rem', color: '#7C3AED' }}>{selectedStaff.workingHours}</Typography></Box>
                      </Box>
                    </Card>
                  </Grid>
                ...
                <Grid container spacing={1.5}>
                  <Grid size={{ xs: 6, sm: 3 }}>
                    <Button fullWidth variant="contained" onClick={() => setIsAssignScheduleOpen(true)} startIcon={<Clock size={14} />} sx={{ borderRadius: '10px', backgroundColor: '#7C3AED', color: '#FFFFFF', fontWeight: 700, textTransform: 'none', fontSize: '0.78125rem', '&:hover': { backgroundColor: '#6D28D9' } }}>
                      Assign Schedule
                    </Button>
                  </Grid>
                  <Grid size={{ xs: 6, sm: 3 }}>
            ...
            {/* TAB 1: WEEKLY SCHEDULE TIMETABLE */}
            {activeDrawerTab === 1 && (
              <Box sx={{ pb: 2 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                  <Box>
                    <Typography variant="subtitle1" sx={{ fontWeight: 800, fontSize: '0.95rem' }}>
                      Weekly Working Schedule ({selectedStaff.name})
                    </Typography>
                    <Typography variant="caption" color="text.secondary">
                      Assigned working shifts must stay within store opening and closing hours.
                    </Typography>
                  </Box>
                  <Button
                    variant="contained"
                    ...
                    onClick={() => setIsAssignScheduleOpen(true)}
                    sx={{ borderRadius: '10px', backgroundColor: '#7C3AED', textTransform: 'none', fontWeight: 700, fontSize: '0.78125rem' }}
                  >
                    Edit Working Hours
                  </Button>
                </Box>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                    {weeklySchedule.map((item) => (
                      <Box key={item.dayOfWeek} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: 1.2, borderRadius: '10px', backgroundColor: item.isWorking ? '#FAFAFC' : '#FEF2F2' }}>
                        ...
                        {item.isWorking ? (
                          <Chip label={`${item.startTime} - ${item.endTime}`} size="small" sx={{ backgroundColor: 'rgba(124, 58, 237, 0.1)', color: '#7C3AED', fontWeight: 700 }} />
                        ) : (
                          <Chip label="Weekly Off" size="small" color="error" variant="outlined" sx={{ fontWeight: 600 }} />
                        )}
                      </Box>
                    ))}
                  </Box>
            ...
            {activeDrawerTab > 1 && (
              <Box sx={{ py: 4, textAlign: 'center' }}>
                <Typography variant="body2" color="text.secondary">
                  Management panel for this tab is loaded. Click <strong>Quick Actions</strong> below or edit via section buttons.
                </Typography>
              </Box>
            )}
          </Drawer>
          ...
          {/* ========================================================================= */}
          <Dialog
            open={isAssignScheduleOpen}
            onClose={() => setIsAssignScheduleOpen(false)}
            maxWidth="sm"
            fullWidth
            slotProps={{ paper: { sx: { borderRadius: '20px', p: 1 } } }}
          >
            <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontWeight: 800, fontSize: '1.1rem' }}>
              Assign Working Hours ({selectedStaff?.name})
              <IconButton onClick={() => setIsAssignScheduleOpen(false)} size="small"><X size={18} /></IconButton>
            </DialogTitle>
            <DialogContent dividers sx={{ border: 'none' }}>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 2.5, fontSize: '0.8125rem' }}>
                Set weekly working shift times for <strong>{selectedStaff?.name}</strong>. Shifts must remain within store operating hours.
              </Typography>
                    ...
                    >
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, minWidth: 140 }}>
                        <Switch
                          ...
                          onChange={() => handleToggleScheduleWorking(idx)}
                          sx={{
                      ...
                      {item.isWorking ? (
                        ...
                        <Chip label="Day Off" size="small" color="error" variant="outlined" sx={{ fontWeight: 600, fontSize: '0.72rem' }} />
                      )}
                    </Box>
                  </Grid>
                ))}
            ...
            <DialogActions sx={{ p: 2 }}>
              <Button onClick={() => setIsAssignScheduleOpen(false)} sx={{ color: '#6B7280', textTransform: 'none', fontWeight: 700 }}>
                Cancel
              </Button>
              <Button
                variant="contained"
                onClick={handleSaveScheduleTimetable}
