...
import { Scissors, MapPin, ArrowLeft } from 'lucide-react';
import { GuestGuard } from '@/shared/components/auth/GuestGuard';

export default function SelectOutletPage() {
  const [selectedOutlet, setSelectedOutlet] = useState('rajapark');

  const outlets = [
    { id: 'rajapark', name: 'Rajapark Branch', location: 'Rajapark, Jaipur' },
    { id: 'malviya', name: 'Malviya Nagar Branch', location: 'Malviya Nagar, Jaipur' },
    { id: 'cscheme', name: 'C-Scheme Branch', location: 'C-Scheme, Jaipur' },
    { id: 'vaishali', name: 'Vaishali Nagar Branch', location: 'Vaishali Nagar, Jaipur' },
  ];
      ...
      >
        ...
        >
          ...
          <Typography variant="body2" color="#6B7280" sx={{ fontSize: '0.875rem', mb: 3.5 }}>
            Choose the outlet you want to access.
          </Typography>

          {/* Outlets List */}
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, mb: 3.5 }}>
            {outlets.map((o) => {
              const isSelected = selectedOutlet === o.id;
              return (
                <Box
                  key={o.id}
