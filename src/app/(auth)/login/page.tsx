...
function LoginContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const isSuperAdmin = email === 'superadmin@sams.com' || email === 'admin@salon.com';

      // 2. Initialize the store so the Axios interceptor picks up the token
      login(loginRes.accessToken, loginRes.refreshToken, {
        id: loginRes.user.id,
        email: loginRes.user.email,
        name: loginRes.user.email ? loginRes.user.email.split('@')[0] : 'User',
        roles: isSuperAdmin ? ['SUPER_ADMIN'] : ['OWNER'],
        permissions: ['*'],
        tenantModules: [],
        isSuperAdmin,
      });

      // 3. Call GET /auth/me to fetch & store full user profile, roles, and permissions
      try {
        ...
        const fullUser = {
          ...
          roles: meRes.roles || loginRes.user.roles || (isSuperAdmin ? ['SUPER_ADMIN'] : ['OWNER']),
          permissions: meRes.permissions || ['*'],
          tenantId: meRes.tenantId || loginRes.user.tenantId,
          tenantModules: meRes.tenantModules || [],
          isSuperAdmin: isSuperAdmin || Boolean((meRes as any)?.isSuperAdmin),
          subscriptionStatus: (meRes as any)?.subscriptionStatus,
          planCode: (meRes as any)?.planCode,
          trialEndsAt: (meRes as any)?.trialEndsAt,
          isTrialExpired: (meRes as any)?.isTrialExpired,
      ...
      try {
        ...
        if (tenantData) {
          useTenantStore.getState().setActiveTenant({
            ...
            logoUrl: '',
            plan: ((useAuthStore.getState().user as any)?.planCode?.toLowerCase() || 'free') as any,
          });
        }
      } catch (tenantErr) {
      ...
      // 5. Redirect to Super Admin Control Plane or Tenant Dashboard
      ...
      let targetDestination = isSuperAdmin ? '/admin/tenants' : redirectTo;
      if (!isSuperAdmin && targetDestination.startsWith('/admin')) {
        ...
        >
          ...
          >
            ...
            >
              <Box
                sx={{
                  ...
                  alignItems: 'center',
                  justifyContent: 'center',
                  boxShadow: '0px 10px 25px rgba(124, 58, 237, 0.2)',
                }}
              >
                <Scissors size={32} />
              </Box>
            ...
            <Box sx={{ textAlign: 'center', mb: 3 }}>
              <Typography
                variant="h4"
                sx={{ fontWeight: 800, color: (t) => (t.palette.mode === 'dark' ? '#F8FAFC' : '#111827') }}
                gutterBottom
              >
                Welcome Back!
              </Typography>
              ...
              <Box sx={{ display: 'flex', gap: 1, justifyContent: 'center', flexWrap: 'wrap' }}>
                <Chip
                  label="Super Admin: superadmin@sams.com"
                  size="small"
                  onClick={() => {
                    setEmail('superadmin@sams.com');
                  ...
                  sx={{
                    ...
                    fontSize: '0.72rem',
                    cursor: 'pointer',
                    '&:hover': { backgroundColor: 'rgba(124, 58, 237, 0.25)', color: '#A78BFA' },
                  }}
                />
                <Chip
              ...
              <Box sx={{ mb: 2.5 }}>
                <Typography
                  ...
                  sx={{
                    ...
                    mb: 0.8,
                    display: 'block',
                    color: (t) => (t.palette.mode === 'dark' ? '#E2E8F0' : '#374151'),
                  }}
                >
                  Email Address
                </Typography>
                      ...
                      endAdornment: (
                        <InputAdornment position="end">
                          <IconButton
                            ...
                            edge="end"
                            size="small"
                            sx={{ color: (t) => (t.palette.mode === 'dark' ? '#94A3B8' : '#9CA3AF') }}
                          >
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      ...
                      sx={{
                        color: (t) => (t.palette.mode === 'dark' ? 'rgba(255, 255, 255, 0.3)' : '#D1D5DB'),
                  ...
                  label={
                    <Typography
                      ...
                      sx={{
                        fontSize: '0.875rem',
                        color: (t) => (t.palette.mode === 'dark' ? '#CBD5E1' : '#4B5563'),
                      }}
                    >
                      Remember me
                    </Typography>
              ...
              <Button
                ...
                sx={{
                  ...
                  '&:hover': {
                    ...
                    boxShadow: '0px 10px 24px rgba(124, 58, 237, 0.45)',
                  },
                }}
              >
            ...
            <Divider
              sx={{
                my: 3,
                color: (t) => (t.palette.mode === 'dark' ? '#64748B' : '#9CA3AF'),
                borderColor: (t) => (t.palette.mode === 'dark' ? 'rgba(255, 255, 255, 0.08)' : undefined),
                fontSize: '0.8125rem',
              }}
            >
              or continue with
            ...
            <Box sx={{ display: 'flex', gap: 2, mb: 3.5 }}>
              <Button
                ...
                sx={{
                  ...
                  borderRadius: '12px',
                  borderColor: (t) => (t.palette.mode === 'dark' ? 'rgba(255, 255, 255, 0.12)' : '#E5E7EB'),
                  color: (t) => (t.palette.mode === 'dark' ? '#F8FAFC' : '#374151'),
                  fontSize: '0.875rem',
                  fontWeight: 600,
                  textTransform: 'none',
                  '&:hover': {
            ...
            <Box sx={{ textAlign: 'center' }}>
              <Typography variant="body2" sx={{ color: (t) => (t.palette.mode === 'dark' ? '#94A3B8' : '#6B7280') }}>
