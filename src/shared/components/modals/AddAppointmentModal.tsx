...
export function AddAppointmentModal({ open, onClose }: AddAppointmentModalProps) {
  ...
  const createAppointmentMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      showToast.success(
        'Appointment Created',
        `Appointment scheduled for ${data?.customer?.name || 'Customer'}`
      );
    ...
    onError: (err: any) => {
      const rawMsg = err?.response?.data?.message || err?.message || 'Failed to create appointment';
      const formattedMsg = Array.isArray(rawMsg) ? rawMsg.join(', ') : rawMsg;
      showToast.error('Failed to Create Appointment', formattedMsg);
    },
  });
